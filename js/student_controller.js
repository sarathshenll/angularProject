...
myApp.controller('student_contrloer', function ($scope, $state, $http, $location) {
    ...
    this.loadData = function (page_number) {
        ...
        $http.get('server/select.php?page=' + page_number + '&search_input=' + search_input).then(function (response) {
...
//
    ...
    this.addStudent = function (info) {
        $http.post('server/insert.php', info).then(function (response) {
    ...
    this.edit_student_info = function (student_id) {
        $http.get('server/selectone.php?student_id=' + student_id).then(function (response) {
    ...
    this.updateStudent = function () {
        $http.put('server/update.php', this.student_info).then(function (response) {
            vm.msg = response.data.message;
            vm.alert_class = 'custom-alert';
    ...
    this.delete_student_info = function (student_id) {
        $http.delete('server/delete.php?student_id=' + student_id).then(function (response) {
            vm.msg = response.data.message;
            vm.alert_class = 'custom-alert';
            vm.loadData($scope.currentPage);
        });
